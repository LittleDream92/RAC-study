...
参考资料（感谢）：[原文地址](https://github.com/shuaiwang007/RAC)
...
```
	...
	总结：核心：
		...
		实现思路：当一个信号被订阅，创建订阅者，并把nextBlock保存到订阅者里面。创建的时候会返回 [RACDynamicSignal
...
```
	...
	总结：我们完全可以用RACSubject代替代理/通知，确实方便许多 这里我们点击TwoViewController的pop的时候 将字符串"ws"传给了ViewController的button的title。
...
###RACMulticastConnection
####使用场景： 当有多个订阅者，但是我们只想发送一个信号的时候怎么办？这时我们就可以用RACMulticastConnection，来实现。
